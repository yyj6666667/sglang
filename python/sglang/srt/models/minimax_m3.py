...
class MiniMaxM3MLP(nn.Module):
    def __init__(
        ...
        if hidden_act == "silu":
            ...
            @torch.compile
            def _swiglu_no_interleaved_with_alpha_and_limit(x, gemm1_alpha, gemm1_limit):
                gate, up = x.chunk(2, dim=-1)
                gate = gate.clamp(min=None, max=gemm1_limit)
                up = up.clamp(min=-gemm1_limit, max=gemm1_limit)
                return gate * torch.sigmoid(gate * gemm1_alpha) * (up + 1)

            self.act_fn = lambda x: _swiglu_no_interleaved_with_alpha_and_limit(
    ...
    def forward(
        ...
        if forward_batch.can_run_tbo:
            ...
            for i in range(self.start_layer, self.end_layer):
                ctx = (
                    nullcontext()
                    if not get_global_server_args().enable_piecewise_cuda_graph
