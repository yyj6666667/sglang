...
#pragma unroll
      ...
      // Always write rotated values back to `input` (this is q itself when
      // load_q==true, or k itself when load_q==false). Additionally, when
      // processing k, also write the rotated value to k_cache for future decode.
...
#pragma unroll
      ...
      // Always write rotated value back to `input` (q or k itself), and
      // additionally to k_cache when processing k.
      store_as<InputStorage>(input, input_vec, lane_id);
      if (!load_q) {
        const auto k_out = pointer::offset(k_cache, loc * cache_stride_bytes, head_id * head_stride_bytes);
